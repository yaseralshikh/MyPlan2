<div>
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="mb-2 row">
                <div class="col-sm-6">
                    <h1 class="m-0">@lang('site.job_types')</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">@lang('site.dashboard')</a>
                        </li>
                        <li class="breadcrumb-item active">@lang('site.job_types')</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header bg-light">
                    <h3 class="card-title">
                        <button wire:click.prevent='addNewJobType' class="ml-1 btn btn-sm btn-primary" {{  auth()->user()->hasPermission('job_types-create') ? '' : 'disabled' }}>
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn btn-primary btn-sm" {{  auth()->user()->hasPermission('job_types-read') ? '' : 'disabled' }}>@lang('site.action')</button>
                            <button type="button" class="btn btn-primary btn-sm dropdown-toggle dropdown-icon"
                                {{  auth()->user()->hasPermission('job_types-read') ? '' : 'disabled' }}
                            ...
                            <div class="dropdown-menu" role="menu" style="">
                                ...
                                <a class="dropdown-item  {{ $selectedRows && auth()->user()->hasPermission('job_types-delete') ? 'text-danger' : 'disabled-link' }}  delete-confirm"
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <button wire:click.prevent="edit({{ $job }})"
                                                class="btn btn-primary btn-sm" {{  auth()->user()->hasPermission('job_types-update') ? '' : 'disabled' }}>
                                                <i class="fa fa-edit"></i>
                                            </button>

                                            <button wire:click.prevent="confirmJobTypeRemoval({{ $job->id }})"
                                                class="btn btn-danger btn-sm" {{  auth()->user()->hasPermission('job_types-delete') ? '' : 'disabled' }}>
                ...
                <div class="card-footer bg-light">
                    {!! $job_types->appends(request()->all())->links() !!}
                </div>
                <!-- /.card-footer-->
            </div>
            <!-- /.row (main row) -->
